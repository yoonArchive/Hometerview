<template>
  <div v-if="!isloading" style="background-color: #F3F4FF;">
      ...
      <div v-if="studySpaceDetail.detailCounts[studentindex] == 0">
        <div class="no-cover-letter-me">
          <div class="d-flex justify-content-between" style="align-items: center;">
            ...
            <button type="button" class="btn" data-bs-toggle="modal" data-bs-target="#exampleModal" style="background-color:#653FD3; color: white;">자소서 바꾸기</button>
          </div>
        </div>
      </div>
    ...
    <div v-else-if="studySpaceDetail.detailCounts[studentindex] == 0 && isMyInfo"></div>
    <div v-else>
      <div class="member-cover-letter-title">
        <div class="row">
          <div class="col-8">
            <div class="member-cover-letter-comname" v-if="studySpaceDetail.comName!=null">[{{studySpaceDetail.comName}}]</div>
            <div class="member-cover-letter-stdname">{{studySpaceDetail.stdName}} 자소서</div>
          </div>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" @click="saveStudyCoverLetterV(selresume)" data-bs-dismiss="modal">저장</button>
        </div>
      </div>
    </div>
  </div>
